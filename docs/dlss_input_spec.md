...
This document details the technical requirements for the EXR buffers passed to DLSS-RR.
...
| **Diffuse Albedo** | `DiffCol` | RGB | `R16G16B16A16_SFLOAT` | Surface diffuse color (no lighting). |
| **Specular Albedo** | `GlossCol` | RGB | `R16G16B16A16_SFLOAT` | Surface specular color/intensity. |
...
- **Specular Albedo**: Flat Black (0.0, 0.0, 0.0)
...
DLSS-RR expects **Current to Previous** motion vectors.
...
- **Single Frame Quality**: DLSS-RR is a temporal denoiser. Evaluation of a single isolated frame will result in significantly lower quality than a sequence.
