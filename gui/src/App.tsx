...
function AppContent() {
  ...
  useEffect(() => {
    // Listen for IPC errors for toast
    window.dlssApi.onError((msg: string) => {
      setToastError(msg);
  ...
  return (
    <div data-testid="app-root" className="h-screen bg-gray-900 text-gray-100 overflow-hidden flex flex-col relative">
      ...
      {toastError && (
        <div className="fixed bottom-4 right-4 z-50 bg-red-600 text-white px-4 py-2 rounded shadow-lg">
          {toastError}
        </div>
      )}
    </div>
